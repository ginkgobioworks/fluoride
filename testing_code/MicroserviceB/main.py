...
@app.route('/')
def main():
    content = urllib.request.urlopen('<HTTP/HTTPS endpoint for microservice A>/get_message').read().decode('utf-8')
    return content, 200, {'ContentType': 'application/json'}

@app.route('/get_message')
def message():
    return jsonify({'message': 'Here is a message from microservice B!'})
